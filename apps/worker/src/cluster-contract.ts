...
export function validateClusterContract(inputReviewIds: string[], candidate: unknown): ValidatedClusterContract {
  ...
  const normalizedExtractions = extractions.map((item, index) => {
    ...
    if (!CLUSTER_CATEGORIES.includes(category as ClusterCategory)) throw new Error(`invalid category: ${category}`);
    return {
      reviewId,
  ...
  const normalizedClusters = clusters.map((item, index) => {
    ...
    if (!CLUSTER_SEVERITIES.includes(severity as ClusterSeverity)) throw new Error(`invalid severity: ${severity}`);
